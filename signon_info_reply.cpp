                                                     ...
                                                     UCalendar* cal)
{
    ...
    std::int32_t year = packet.get<std::uint16_t>(off + 6);
    std::int32_t month = packet.get<std::uint8_t>(off + 8) - 1;
    std::int32_t day = packet.get<std::uint8_t>(off + 9);
    std::int32_t hour = packet.get<std::uint8_t>(off + 10);
    std::int32_t minute = packet.get<std::uint8_t>(off + 11);
    std::int32_t second = packet.get<std::uint8_t>(off + 12);
...
signon_info_reply::signon_info_reply(const access_input_packet& packet)
{
    ...
    ccsid_ = static_cast<std::uint16_t>((off == access_input_packet::npos) ? 0 : packet.get<std::uint32_t>(off + 6));
